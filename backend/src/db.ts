...
export function migrate() {
  ...
  // Perform legacy upgrade (conversation -> profile) BEFORE executing schema so that
  // index creation statements referencing profile_* columns do not fail on old DBs.
  try {
    upgradeLegacyConversationSchemaPreSchema();
  } catch (e) {
    console.warn('Pre-schema legacy upgrade failed (continuing):', e);
  }
  ...
  console.log('Running default profile upgrade...');
  upgradeDefaultProfileIfNeeded();
  console.log('Default profile upgrade completed');
...
// Add profile_id columns / rename legacy conversation tables before running schema.sql
function upgradeLegacyConversationSchemaPreSchema(): void {
  console.log('Running pre-schema legacy conversation->profile upgrade...');
  db.exec('PRAGMA foreign_keys = OFF;');
  try {
    const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all() as any[];
    const tableNames = new Set(tables.map(t => t.name));

    // Rename conversations -> profiles if needed
    if (tableNames.has('conversations') && !tableNames.has('profiles')) {
      console.log('Renaming table conversations -> profiles');
      db.prepare('ALTER TABLE conversations RENAME TO profiles').run();
    }
    // Refresh table names after potential rename
    {
      const t2 = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all() as any[];
      for (const r of t2) tableNames.add(r.name);
    }
    // Rename conversation_members -> profile_members if needed
    if (tableNames.has('conversation_members') && !tableNames.has('profile_members')) {
      console.log('Renaming table conversation_members -> profile_members');
      db.prepare('ALTER TABLE conversation_members RENAME TO profile_members').run();
    }

    const candidateTables = ['messages','health_data','medications','reports','reminders','activities'];
    for (const tbl of candidateTables) {
      if (!tableNames.has(tbl)) continue;
      ...
      if (hasConversationId && !hasProfileId) {
        console.log(`Adding profile_id column to ${tbl} and backfilling from conversation_id`);
        try {
          db.prepare(`ALTER TABLE ${tbl} ADD COLUMN profile_id TEXT`).run();
          db.prepare(`UPDATE ${tbl} SET profile_id = conversation_id WHERE profile_id IS NULL OR profile_id = ''`).run();
        } catch (e) {
          console.warn(`Failed adding/backfilling profile_id for ${tbl}:`, e);
        }
      }
    }
  } finally {
  ...
  console.log('Pre-schema legacy upgrade complete');
}
...
function upgradeReportsTableIfNeeded(): void {
    ...
    if (needsRebuild) {
      ...
      const createTableSql = `
        CREATE TABLE IF NOT EXISTS reports (
          ...
          profile_id TEXT, -- nullable, if report came from a profile chat
  ...
  db.prepare('CREATE INDEX IF NOT EXISTS idx_reports_profile ON reports(profile_id)').run();
      ...
      for (const row of backupData) {
        try {
          ...
          db.prepare(`
            INSERT INTO reports (id, user_id, profile_id, file_path, file_type, source, ai_summary, created_at, parsed)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).run(
            row.id,
            row.user_id,
            row.profile_id || row.conversation_id || null, // migrate old conversation_id if present
            row.file_path,
            fileType,
            source,
            row.ai_summary,
...
function upgradeDefaultProfileIfNeeded(): void {
  try {
    console.log('Checking if default profile title needs update...');
    // Profiles table now stores the default chat entity
    const defaultProfile = db.prepare(`
      SELECT id, title FROM profiles 
      WHERE (title = 'Default' OR id = 'default-profile')
      AND is_default = 1
    `).get() as any;

    if (defaultProfile) {
      console.log('Found default profile with title:', defaultProfile.title);
      if (defaultProfile.title === 'Default') {
        console.log('Updating default profile title from "Default" to "Me"...');
        db.prepare('UPDATE profiles SET title = ? WHERE id = ?')
          .run('Me', defaultProfile.id);
        console.log('Successfully updated default profile title to "Me"');
      } else {
        console.log('Default profile title is already correct:', defaultProfile.title);
      }
    } else {
      console.log('No default profile found to update');
    }
  } catch (e) {
    console.warn('default profile upgrade skipped', e);
  }
}
