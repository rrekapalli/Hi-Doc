...
class DatabaseService {
  ...
  static const _dbVersion =
      10; // v9: rename tables; v10: unify prototype user ids
  ...
  Future<String> sendProfileMessage({
    ...
    final r = await http.post(
      Uri.parse('$_backendBaseUrl/api/profiles/$profileId/messages'),
      headers: await _getAuthHeaders(),
      body: jsonEncode({'content': content, 'contentType': contentType}),
    );
  ...
  Future<String> createProfile({
    ...
    final r = await http.post(
      Uri.parse('$_backendBaseUrl/api/profiles'),
      headers: await _getAuthHeaders(),
      body: jsonEncode({'title': title, 'type': type, 'memberIds': memberIds}),
    );
  ...
  Future<void> init() async {
    ...
    try {
      if (kIsWeb) {
        ...
        if (kDebugMode) {
          debugPrint(
            '[DB] Opened web IndexedDB database name=$_dbName origin=${Uri.base.origin} version=$_dbVersion',
          );
        }
        ...
        if (kDebugMode) {
          debugPrint(
            '[DB] Opened desktop SQLite database $_dbName at $fullPath (version=$_dbVersion)',
          );
        }
      } else {
        ...
        if (kDebugMode) {
          debugPrint(
            '[DB] Opened mobile SQLite database $_dbName at $fullPath (version=$_dbVersion)',
          );
        }
      }
      _initialized = true;
      if (kDebugMode) {
        try {
          await _debugDumpMedicationCounts();
        } catch (e) {
          debugPrint('[DB] debug dump failed: $e');
        }
      }
    } catch (e, st) {
  ...
  Database _ensure() {
    ...
    if (!_initialized || db == null || !db.isOpen) {
      throw StateError('DB not initialized');
    }
    return db;
  }
  ...
  Future<void> _onCreate(Database db) async {
    await db.execute(
      '''CREATE TABLE IF NOT EXISTS messages (id TEXT PRIMARY KEY, role TEXT, content TEXT, created_at INTEGER, person_id TEXT)''',
    );
    await db.execute(
      '''CREATE TABLE IF NOT EXISTS health_entries (id TEXT PRIMARY KEY, person_id TEXT, timestamp INTEGER NOT NULL, type TEXT NOT NULL, data TEXT NOT NULL)''',
    );
    await db.execute(
      '''CREATE TABLE IF NOT EXISTS reports (id TEXT PRIMARY KEY, file_path TEXT, type TEXT, data TEXT, upload_date INTEGER)''',
    );
    await db.execute(
      '''CREATE TABLE IF NOT EXISTS reminders (id TEXT PRIMARY KEY, user_id TEXT, medication_id TEXT, title TEXT, time TEXT, message TEXT, repeat TEXT, days TEXT, active INTEGER DEFAULT 1)''',
    );
    await db.execute(
      '''CREATE TABLE IF NOT EXISTS parsed_parameters (id TEXT PRIMARY KEY, unit TEXT, datetime TEXT, raw_json TEXT)''',
    );
  ...
  Future<void> _migrateUnifyPrototypeUser(Database db) async {
    try {
      final changed = await db.rawUpdate(
        "UPDATE medications SET user_id = 'prototype-user' WHERE user_id IN ('prototype-user-12345','prototype-user-1234','prototype-user-123')",
      );
      if (kDebugMode) {
        debugPrint(
          '[DB] v10 migration unified user ids; rows updated: $changed',
        );
      }
    } catch (e) {
  ...
  MessageRole _roleFromString(String r) => switch (r.toLowerCase()) {
  ...
  Future<List<HealthEntry>> listAllEntries({String? personId}) async {
    ...
    return rows
        .map((r) => HealthEntry.fromJson(_parseMap(r['data'] as String)))
        .toList();
  }
  ...
  Future<String> createMedication(Map<String, dynamic> data) async {
    if (kIsWeb) {
      ...
      final r = await http.post(
        Uri.parse('$_backendBaseUrl/api/medications'),
        headers: await _getAuthHeaders(),
        body: jsonEncode(payload),
      );
      if (r.statusCode != 200 && r.statusCode != 201) {
        throw Exception('Create medication failed ${r.statusCode} ${r.body}');
      }
  ...
  Future<void> updateMedication(Map<String, dynamic> data) async {
    if (kIsWeb) {
      ...
      final r = await http.put(
        Uri.parse('$_backendBaseUrl/api/medications/$id'),
        headers: await _getAuthHeaders(),
        body: jsonEncode(payload),
      );
      if (r.statusCode != 200) {
        throw Exception('Update medication failed ${r.statusCode}');
      }
      await _cacheMedicationLocal({
        'id': id,
        'user_id': data['user_id'] ?? 'prototype-user',
  ...
  Future<List<Map<String, dynamic>>> listMedications({
    ...
    if (kIsWeb) {
      final uri = Uri.parse(
        '$_backendBaseUrl/api/medications',
      ).replace(queryParameters: {'profile_id': profileId});
      final r = await http.get(uri, headers: await _getAuthHeaders());
  ...
  Future<String> createSchedule(Map<String, dynamic> data) async {
    if (kIsWeb) {
      ...
      final r = await http.post(
        Uri.parse('$_backendBaseUrl/api/medications/$medId/schedules'),
        headers: await _getAuthHeaders(),
        body: jsonEncode(payload),
      );
  ...
  Future<void> updateSchedule(Map<String, dynamic> data) async {
    if (kIsWeb) {
      ...
      final r = await http.put(
        Uri.parse('$_backendBaseUrl/api/schedules/$id'),
        headers: await _getAuthHeaders(),
        body: jsonEncode(payload),
      );
  ...
  Future<List<Map<String, dynamic>>> getSchedules(String medicationId) async {
    if (kIsWeb) {
      final r = await http.get(
        Uri.parse('$_backendBaseUrl/api/medications/$medicationId'),
        headers: await _getAuthHeaders(),
      );
  ...
  Future<Map<String, dynamic>?> getScheduleById(String scheduleId) async {
    ...
    final rows = await _ensure().query(
      'medication_schedules',
      where: 'id = ?',
  ...
  Future<String> createScheduleTime(Map<String, dynamic> data) async {
    if (kIsWeb) {
      ...
      if (_scheduleIdMap.containsKey(scheduleId)) {
        ...
        data['schedule_id'] =
            scheduleId; // keep consistency if later cached locally
      }
      final payload = {
      ...
      final r = await http.post(
        Uri.parse('$_backendBaseUrl/api/schedules/$scheduleId/times'),
        headers: await _getAuthHeaders(),
        body: jsonEncode(payload),
      );
  ...
  Future<void> updateScheduleTime(Map<String, dynamic> data) async {
    if (kIsWeb) {
      ...
      final r = await http.put(
        Uri.parse('$_backendBaseUrl/api/schedule-times/$id'),
        headers: await _getAuthHeaders(),
        body: jsonEncode(payload),
      );
  ...
  Future<void> insertIntakeLog(Map<String, dynamic> data) async {
    if (kIsWeb) {
      final r = await http.post(
        Uri.parse(
          '$_backendBaseUrl/api/schedule-times/${data['schedule_time_id']}/intake-logs',
        ),
        headers: await _getAuthHeaders(),
        body: jsonEncode({
          'status': data['status'],
  ...
  Future<List<Map<String, dynamic>>> listIntakeLogs(
    String medicationId, {
    int? fromTs,
    int? toTs,
  }) async {
    if (kIsWeb) {
      ...
      if (fromTs != null) params['from'] = fromTs.toString();
      if (toTs != null) params['to'] = toTs.toString();
      final uri = Uri.parse(
        '$_backendBaseUrl/api/medications/$medicationId/intake-logs',
    ...
    return _ensure().rawQuery('''SELECT mil.* FROM medication_intake_logs mil
  ...
  Future<List<Map<String, dynamic>>> rawQuery(
    String sql, [
    List<Object?>? args,
  ]) async => _ensure().rawQuery(sql, args);

  // ---- Reminders ----
  Future<void> insertReminder(Map<String, dynamic> reminder) async {
    const requiredFields = ['id', 'medication_id', 'title', 'time', 'repeat'];
    for (final f in requiredFields) {
      if (!reminder.containsKey(f)) {
    ...
    if (!kIsWeb) {
      await _notificationService.scheduleMedicationReminder(
        ...
        message:
            (reminder['message'] as String?) ?? reminder['title'] as String,
  ...
  Future<void> updateMedicationReminders(String medicationId) async {
    ...
    for (final r in reminders) {
      await _notificationService.cancelNotification(r['id'] as String);
    }
    for (final r in reminders.where((x) => x['active'] == 1)) {
      await _notificationService.scheduleMedicationReminder(
        id: r['id'] as String,
  ...
  Future<void> _debugDumpMedicationCounts() async {
    ...
    debugPrint(
      '[DB] Medication tables after open: medications=$meds schedules=$sched times=$times intake_logs=$logs',
    );
