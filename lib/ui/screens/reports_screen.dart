...
class _ReportsScreenState extends State<ReportsScreen> {
    ...
    try {
      ...
      if (image != null) {
        if (kIsWeb) {
          ...
          await _processSelectedFile(File(image.path), ReportSource.camera,
              originalFileName:
                  'Camera_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.jpg');
        }
      }
      ...
      if (image != null) {
        if (kIsWeb) {
          ...
          await _processSelectedFileBytes(
              bytes, image.name, ReportSource.upload);
        } else {
          await _processSelectedFile(File(image.path), ReportSource.upload,
    ...
    try {
      ...
      if (result != null) {
        ...
        if (kIsWeb) {
          if (file.bytes != null) {
            await _processSelectedFileBytes(
                file.bytes!, file.name, ReportSource.upload);
          } else {
            _showErrorSnackBar('Failed to read file data');
          }
        } else {
          if (file.path != null) {
            await _processSelectedFile(File(file.path!), ReportSource.upload,
  ...
  Future<void> _processSelectedFile(File file, ReportSource source,
  ...
  Future<void> _processSelectedFileBytes(
      Uint8List bytes, String fileName, ReportSource source) async {
    try {
      final rp = context.read<ReportsProvider>();
      final messenger = ScaffoldMessenger.of(context);
      const userId = 'prototype-user';
  ...
  Future<void> _deleteReport(Report report) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (c) => AlertDialog(
        ...
        content:
            Text('Are you sure you want to delete "${report.displayName}"?'),
        actions: [
          TextButton(
                    ...
                    child: Column(
                        ...
                        children: [
                          ...
                          Text('No reports yet',
                              style: Theme.of(context).textTheme.headlineSmall),
                          const SizedBox(height: 8),
                          Text('Scan, upload, or share reports to get started',
                              style: Theme.of(context)
                                  .textTheme
                                  .bodyMedium
                                  ?.copyWith(
            ...
            Row(children: [
              Container(
                ...
                decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(8)),
                child:
                    Text(report.typeIcon, style: const TextStyle(fontSize: 20)),
              ),
              ...
              PopupMenuButton<String>(
                onSelected: (v) {
                  if (v == 'delete') _deleteReport(report);
                },
                itemBuilder: (c) => const [
                  PopupMenuItem(
                      value: 'delete',
