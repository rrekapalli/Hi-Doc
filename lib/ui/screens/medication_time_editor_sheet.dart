...
class MedicationTimeEditorSheet extends StatefulWidget {
  ...
  @override
  State<MedicationTimeEditorSheet> createState() =>
      _MedicationTimeEditorSheetState();
}
        ...
        children: [
          ...
          TextField(
            ...
            decoration: const InputDecoration(
              labelText: 'Dosage (e.g. 1 tab 500 mg)',
            ),
          ),
          const SizedBox(height: 8),
          TextField(
            ...
            decoration: const InputDecoration(
              labelText: 'Instructions (before breakfast etc.)',
            ),
          ),
          SwitchListTile(
            value: _prn,
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: (_time == null && !_prn)
                  ? null
                  : () async {
                      final db = context.read<DatabaseService>();
                      final navigator = Navigator.of(context);
                      final scheduleRow = await db.getScheduleById(
                        widget.scheduleId,
                      );
                      if (scheduleRow == null) {
                      ...
                      final repo = MedicationsRepositoryBridge(
                        reminderService: ReminderService(db),
                      );
                      final formatted = _time == null
                          ? '00:00'
                          : '${_time!.hour.toString().padLeft(2, '0')}:${_time!.minute.toString().padLeft(2, '0')}';
                      final timeRow = MedicationScheduleTime.create(
                        scheduleId: widget.scheduleId,
                        timeLocal: formatted,
                        dosage: _dosageCtrl.text.isEmpty
                            ? null
                            : _dosageCtrl.text,
