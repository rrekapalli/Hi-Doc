...
void main() async {
  ...
  await Future.wait([if (!kIsWeb) notificationService.init(), db.init()]);

  // Initialize repository manager after database is ready
    ...
    return MultiProvider(
      providers: [
        ...
        ChangeNotifierProvider(
          create: (context) =>
              ChatProvider(db: db, authService: context.read<AuthService>()),
        ),
        ...
        ChangeNotifierProxyProvider<ChatProvider, ActivitiesProvider>(
          create: (context) =>
              ActivitiesProvider(chatProvider: context.read<ChatProvider>()),
          update: (context, chat, previous) =>
              previous ?? ActivitiesProvider(chatProvider: chat),
        ),
        ChangeNotifierProvider(
          create: (context) => TrendsProvider(
            service: HealthTrendsService(context.read<AuthService>()),
          ),
        ),
      ],
      child: Builder(
                ...
                return Container(
                  decoration: const BoxDecoration(
                    gradient: RadialGradient(
                      ...
                      radius: 1.2,
                      colors: [Color(0xFFE8F3FF), Colors.white],
                      stops: [0, 0.65],
                    ),
                  ),
                  child: child,
